...
start <- as.Date('2018-02-22')
end <- as.Date('2021-02-22')
...
descdist(as.numeric(stock.returns)) # 查看接近那种分布

# 具体一些检验
library(nortest)
# Shapiro-Wilks test
shapiro.test(as.numeric(stock.returns)) # p !> 0.1，所以不服从正态分布

# Kolmogorov-Smirnov test
ks.test(stock.returns,
        "pnorm",
        mean(stock.returns),
        sqrt(var(stock.returns)))

# Cramer-Von Mises test
cvm.test(stock.returns)
...
sigma <- fit.t$estimate[2]/100 # s
nu <- fit.t$estimate[3] # df
VaR.t <- sigma * qt(df=nu, p=0.05) 
VaR.t

# VaR加标注线
hist(stock.returns, breaks=50, freq=FALSE, main='Distribution of Stock Returns')
abline(v=VaR.norm, lwd=1, lty=1)
abline(v=VaR.t, lwd=1.5, lty=3)
legend("topleft", 
       legend = c("VaR.norm","VaR.t"),
       col="black",
       lty=c(1,2))
box()
...
# Parametric ES - Normality Assumption，payoff服从正态分布的假设
# 95%

stock.mean <- mean(stock.returns)
stock.variance <- var(stock.returns)
horizon <- 1
confidence <- qnorm(0.05) # -1.644854
stock.VaR.p <- -(stock.mean*horizon+confidence*sqrt(horizon*stock.variance))
...
# risk confidence sensitivity of daily returns
chart.VaRSensitivity(stock.returns,
                     methods=c("GaussianVaR", "ModifiedVaR", "HistoricalVaR", 
                               "GaussianES", "ModifiedES","HistoricalES"),
                     reference.grid=TRUE, 
...
# Plot Chart Bar of VaR
chart.BarVaR(stock.returns,
             legend.loc='topleft',
             methods = c("HistoricalES"),
             main="Stock Returns and ES",
             legend.cex = 0.5,
             width=12)

# Drawdowns ---------------------------------------------------------------

# 跌幅分析（measure of the decline from a historical peak）
...
plot(stock.dd*100,
     type='b',
     main='Stock Drawdowns',
     ylab='Drawdown Percent(%)')
